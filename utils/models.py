...
def train_three_models(X_train, y_train):
    ...
    models = {
        ...
        "nn": nn,
        "svm_linear": svm.SVC().fit(X_train,y_train),
...
def train_three_models_lp(X_train, y_train):
    ...
    models = {
        ...
        "nn": nn,
        "svm_linear": svm.SVC(probability=True).fit(X_train,y_train),
        #"svm_rbf": svm.SVC(kernel='rbf').fit(X_train,y_train)
    }
...
def evaluation_test(models, X_test, y_test):
    ...
    svm_linear_pred = models['svm_linear'].predict(X_test)
    ...
    print_eval_states(y_test, svm_linear_pred, name="SVM Linear")
    
    return dt_pred, rfc_pred, nn_pred
...
def print_eval_states(y_test, y_pred, name=None):
    ...
    print(f"Model: name {name}: & {accuracy:.4f}\t& {precision:.4f}\t& {recall:.4f}\t& {f1:.4f}")
...
def save_three_models(models, dataset_name, path='./saved_models'):
    ...
    models['nn'].save(f'{storing_folder}/nn.h5', save_format='h5')
    pickle.dump(models['svm_linear'], open(f'{storing_folder}/svm_linear.p', 'wb'))
...
def save_lp_three_models(models, dataset_name, path='./saved_models'):
    ...
    models['nn'].save(f'{storing_folder}/nn_lp.h5', save_format='h5')
    pickle.dump(models['svm_linear'], open(f'{storing_folder}/svm_linear_lp.p', 'wb'))
...
def load_three_models(num_features, dataset_name, path='./saved_models'):
    ...
    models['svm_linear'] = pickle.load(open(f'{storing_folder}/svm_linear.p', 'rb'))
...
def load_lp_three_models(num_features, dataset_name, path='./saved_models'):
    ...
    models['svm_linear'] = pickle.load(open(f'{storing_folder}/svm_linear_lp.p', 'rb'))
