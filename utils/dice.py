...
def dice_wrap_models(models, cat_to_ohe_cat, ohe_feature_names):
    ...
    return {
        ...
        'svm_linear': RecordWrapper(models['svm_linear'], cat_to_ohe_cat, ohe_feature_names),
    }
...
def get_dice_cfs(data_interface, wrapped_models):
    ...
    return {
        ...
        'svm_linear': dice_ml.Dice(data_interface, dice_ml.Model(model=wrapped_models['svm_linear'], backend="sklearn")),
    }
            ...
            for num_cf in range(num_cf_per_instance):
                ...
                try:
                    exp = dice_cfs[k].generate_counterfactuals(
                        input_query[df_info.feature_names], total_CFs=1, sample_size=sample_size, desired_class="opposite", verbose=False, posthoc_sparsity_param=None)
                except Exception as e:
                    print("Could not find counterfactuals for this instance.")
                    print(num_cf)
        ...
        for i in range(len(results[k])):
            ...
            if not results[k][i]['cf'] is None:
                ...
                scaled_cf_df.loc[0, df_info.target_name] = df_info.target_label_encoder.inverse_transform(
                    [int(scaled_cf_df.loc[0, df_info.target_name])])[0]
                origin_cf_columns = [
            ...
            print(results[k][i]['cf'][df_info.target_name])
            print(results[k][i]['ground_truth'])
            
            final_df['Found'] = ["Y" if not int(results[k][i]['cf'][df_info.target_name]) ==  int(results[k][i]['ground_truth']) else "N"]
