...
def remove_missing_values(df):
    ...
    for col in df.columns:
        if '?' in list(df[col].unique()):
            ...
            # Replace the missing value by the most common.
            most_common = df[col].mode()[0]  # Get the most common value in the column
            df[col] = df[col].replace('?', most_common)  # Replace '?' with the most common value
...
def preprocess_df(df_load_fn):
    ...
    scaled_df[target_name] = target_label_encoder.fit_transform(scaled_df[target_name])
